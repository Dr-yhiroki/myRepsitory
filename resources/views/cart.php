...
<div class="w3-main" style="margin-left:300px">
  ...
  <div class="w3-row-padding">
    <?php foreach(array_unique($cartItems, SORT_REGULAR) as $index => $item): ?>
      <?php $count = array_count_values(array_column($cartItems,'id'))?>
      <div class="w3-third w3-container w3-margin-bottom">
        <img src="/photo/<?= $item->id ?>.jpg" alt="Norway" style="width:100%" class="w3-hover-opacity">
        <div class="w3-container w3-white">
          <?= $item->name ?> &nbsp; <?= $item->price ?> 円
          <a href="/detail/<?= $item->id ?>"> 詳細 </a>

            <form action="/cart/delete/<?=$index?>" method="POST">
              <?php echo("個数:".$count[$item->id]."個");?>
              <?= csrf_field()?>
              &nbsp;&nbsp;
...
<div class="cart_detail">
  <form action="/buy" method="post" name="goukei">
    ...
    <?php endforeach?>
    <p>カート内の合計：<?php echo "$sum"; ?>円 &nbsp;&nbsp;<input type="button" onclick="location.href='/buy'" value="購入手続きへ" class="button"></p>
  </form>
</div>
