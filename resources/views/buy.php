...
<div class="box">
  <div class="purchaselist">
    ...
    <p>購入品：</p>
    <div class="b-list">
      <?php $count = array_count_values(array_column($cartItems,'id'))?>
    <div class="b-name">
        <?php foreach (array_unique($cartItems, SORT_REGULAR) as $item ): ?>
          <p><?= $item->name ?></p>
        <?php $sum = $sum + $item->price * $count[$item->id] ?>
        <?php endforeach?>
    </div>
    <div class="b-money">
      <?php foreach (array_unique($cartItems, SORT_REGULAR) as $item ): ?>
        <?php $sub = $item->price * $count[$item->id] ?>
        <p><?php echo "$sub"; ?> 円</p>
      <?php endforeach?>
    </div>
    <div class="b-count">
      <?php foreach (array_unique($cartItems, SORT_REGULAR) as $item ): ?>
        <p><?php echo("個数:".$count[$item->id]."個 ");?></p>
      <?php endforeach?>
    </div>

    </div>
    <div class="b-sum">
      <p>購入金額：<?php echo "$sum"; ?>円</p>
    </div>
  </div>
  ...
  <div class="input_form">
    <form action="/buy" method="POST">
      <?=csrf_field()?>
      <p>お名前：&emsp;&emsp;&emsp;&emsp;＊<input type="text" name="name" value="<?=old('name')?>"></p>
      <?php if($errors->first('name')):?>
      <p class="errormessage">名前を入力してください。</p>
      <?php endif;?>
      <p>メールアドレス：＊<input type="text" name="email" value="<?=old('email')?>"></p>
      <?php if($errors->first('email')):?>
      <p class="errormessage">Emailを入力してください。</p>
      <?php endif;?>
      <p>電話番号：&emsp;&emsp;&emsp;&emsp;<input type="text" name="tel" value="<?=old('tel')?>"></p>
      <p>住所：&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input type="text" name="address" value="<?=old('address')?>"></p>
